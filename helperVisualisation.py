## Definition of imports and requirements
...
import math
import helperVisualisationDigits
...
def tf_format_mnist_images(X, Y, Y_, n=100, lines=10):
    """
    :param X: tensor [100+, 28, 28, 1] with the batch of images
    :param Y: tensor [100+, 10] of recognised digits (one-hot encoded)
    :param Y_: tensor [100+, 10] of correct digits (one-hot encoded)
    :param n: number of incorrects to display. There is no space for more than 100
    :param lines: number of lines in the matplotlib graph. No more than 10
    :return: tensor [280, 280, 3] containing the 100 first unrecognised images (rgb, uint8)
             followed by other of recognised images.
             100 images max arranged as a 10x10 array.
             Unrecognised are displayed on red and labeled with the correct (left) and recognised digit (right).
    """
    ...
    digits_left = tf.image.grayscale_to_rgb(helperVisualisationDigits.digits_left())
    correct_tags = tf.gather(digits_left, tf.argmax(Ys_, 1))
    digits_right = tf.image.grayscale_to_rgb(helperVisualisationDigits.digits_right())
    computed_tags = tf.gather(digits_right, tf.argmax(Ys, 1))

    superimposed_digits = tf.select(correct_prediction_s, tf.zeros_like(correct_tags),correct_tags+computed_tags)
    correct_bkg   = tf.reshape(tf.tile([1.3,1.3,1.3], [28*28]), [1, 28,28,3])
    incorrect_bkg = tf.reshape(tf.tile([1.3,1.0,1.0], [28*28]), [1, 28,28,3])
    recognised_bkg = tf.gather(tf.concat(0, [incorrect_bkg, correct_bkg]), tf.cast(correct_prediction_s, tf.int32))
...
def probability_distribution(data):
    ...
    Returns n+1 bucket boundaries. Spreads the remainder data. size % n more or less evenly among the central buckets.

    :param data: 1-D ndarray (float)
    :return: 1-D ndarray (size n+1). Contain the bucket boundaries.
    """

    n = HISTOGRAM_BUCKETS #global defined earlier
    data.sort()
